...
class BigramLanguageModel:
    ...
    def compute_bigram_prob_laplace(self, alpha):
        ...
        for i,_ in enumerate(self.bi_prob):
            cnt = np.sum(self.bi_prob[i]) + self.alpha*self.n
            if cnt>0:
                self.bi_prob[i] = (self.bi_prob[i] + self.alpha) / cnt
    ...
    def compute_bigram_prob_interpolation(self, beta, alpha):
        ...
        col_counts = np.sum(self.bi_counts, axis=0)
        corpus_n = np.sum(self.bi_counts)

        for i, _ in enumerate(self.bi_prob):
            self.bi_prob[i] = beta*(self.bi_prob[i]) + (1-beta)*(col_counts+alpha)/(corpus_n+alpha*self.n)
